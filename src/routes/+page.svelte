<script>
    ...
    import {plainText, excerpt, passedDays, formatDate} from '$lib';
	let { data } = $props();
	...
	<section class="hero-grid overflow-hidden border-b border-slate-200 px-5 py-14 sm:py-20">
		<div class="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
			<div>
				...
				>
					<Sparkles class="h-3.5 w-3.5 text-lime-600" aria-hidden="true" />
					Dibuat khusus untuk mahasiswa IT
				</div>

				<h1
					class="mt-6 max-w-3xl text-4xl leading-[1.12] font-extrabold tracking-tight sm:text-5xl lg:text-6xl"
				>
					Mulai karier tech-mu dari
					<span class="text-indigo-600">
						internship yang <span class="hero-highlight">tepat</span>.
					</span>
				</h1>
				...
				<div class="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-xs font-bold text-slate-500">
					<span class="flex items-center gap-2">
						<BadgeCheck class="h-4 w-4 text-indigo-600" aria-hidden="true" />
						Lowongan terkurasi
					</span>

					<span class="flex items-center gap-2">
						<WalletCards class="h-4 w-4 text-indigo-600" aria-hidden="true" />
						Gratis untuk mahasiswa
					</span>

					<span class="flex items-center gap-2">
						<Users class="h-4 w-4 text-indigo-600" aria-hidden="true" />
						Untuk talenta IT
					</span>
				</div>
			</div>
			...
			<div class="relative mx-auto hidden w-full max-w-md lg:block" aria-hidden="true">
				...
				<div
					class="relative rounded-4xl border border-indigo-100 bg-white/80 p-5 shadow-2xl shadow-indigo-200/50 backdrop-blur"
				>
					<div class="mb-4 flex items-center justify-between">
						<div class="flex items-center gap-2 text-xs font-bold text-slate-500">
							<span class="h-2.5 w-2.5 rounded-full bg-lime-400"></span>
							Kesempatan minggu ini
						</div>
	...
	<section id="vacancies" class="px-5 py-14 sm:py-16">
		<div class="mx-auto max-w-6xl">
			<div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
				...
				<p id="result-count" class="text-sm font-bold text-slate-500" aria-live="polite"></p>
			</div>
						...
						>
							<div class="flex flex-wrap gap-4 text-xs font-semibold text-slate-500">
								<span class="flex items-center gap-1.5"
									><MapPin class="h-3.5 w-3.5 text-slate-400" aria-hidden="true" />{vacancy.location}</span
								><span class="flex items-center gap-1.5"
									><Clock3 class="h-3.5 w-3.5 text-slate-400" aria-hidden="true" />{passedDays(vacancy.createdAt)}</span
								>
							</div>
			...
			<div
				id="empty-state"
				class="mt-5 hidden rounded-2xl border border-dashed border-slate-300 py-14 text-center"
			>
				<span
				...
				<button
					id="reset-filter"
					class="mt-5 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white"
				>
